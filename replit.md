...
**Latest Status (July 19, 2025):** **PROFILE TOGGLE FIX & ROUTE CLEANUP**: Fixed profile page visibility toggle buttons that were showing "settings updated" but not visually switching off. Resolved by correcting optimistic update data structure in `updatePickVisibilityMutation`. Eliminated conflicting route registrations between `routes-user-preferences.ts` and `routes-user-picks.ts` that caused 401 Unauthorized errors. Removed duplicate `/picks` routes from user preferences file, keeping only bet unit functionality. Profile toggles now properly switch off/on and maintain visual state after updates.
