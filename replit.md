...
**Lineup System Enhancement (July 18, 2025):** Improved lineup fetching functionality to try multiple MLB API endpoints (linescore, boxscore, content) when official lineups aren't posted yet. Added roster-based fallback system that creates probable lineups from active team players when official lineups are unavailable. Enhanced error handling and logging to provide better visibility into lineup data availability. System now provides lineup information even for games scheduled far in advance.
