...
**Latest Status (July 22, 2025):** **GRADE CALCULATION CONSISTENCY FIXED**: Resolved critical mismatch between displayed analysis factors and BetBot grades. Previously, analysis factors averaged 85-86 (should be B+) but grade showed "A" due to separate calculation methods. Fixed BettingRecommendationEngine.assignGrade() to use same 60-100 scale as analysis factors display - converts edge and confidence to 60-100 range, averages them, then assigns grades (A=90+, B+=85+, B=80+, C+=75+, etc.). Now Kansas City Royals shows grade "A" matching analysis factors averaging 83+ (system confidence 87, market inefficiency 100). Pick persistence also confirmed working - cleared database and regenerated fresh picks that remain stable across refreshes. Both Daily Pick (Under B+) and Lock Pick (Kansas City Royals A) now show grades that logically match their analysis factor displays.
