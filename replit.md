...
**Latest Status (July 19, 2025):** **PICK TILE AUTO-HIDE IMPLEMENTATION**: Implemented automatic tile hiding functionality for Pick of the Day and Logged In Lock when their respective games begin. Both tiles now disappear when game time passes and display "Game Already Started" message with guidance that "New picks will be available at 2:00 AM EST". Tiles remain hidden until the 2 AM rotation generates fresh picks. Uses `shouldHideStartedPick()` function that checks both game start status and time before 2 AM EST. Maintains clean user experience by removing stale picks once games go live.
